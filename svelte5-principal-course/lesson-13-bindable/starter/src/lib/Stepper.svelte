<script lang="ts">
	...
	// Lesson 13 build: change `step = 0` to `step = $bindable(0)` so the
	// parent can two-way bind via <Stepper bind:step />. Without $bindable,
	// the reassignments in prev/next/goTo below would fail at runtime.
