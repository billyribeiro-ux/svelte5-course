<script lang="ts">
	...
	function applyPreset(p: Preset): void {
		// Lesson 08 build: update l/c/h from p (one-shot).
	}
	...
	async function copyCss(): Promise<void> {
		// Lesson 08 build: await navigator.clipboard.writeText(cssExport),
		// set `copied = true` for ~1.5s on success.
	}
...
<div class="grid">
	<section class="controls">
		<fieldset>
			...
			<!-- Lesson 08 build: set the swatch's background with `style:background={accent}`. -->
	...
	<!--
		Lesson 08 build: this section is the token boundary.
		  class   — already wired with the object form (`{{ preview: true, dark: mode === 'dark' }}`).
		            Note how it combines the always-on `preview` with the conditional `dark`.
		  style:  — three custom properties are MISSING. Add:
		              style:--accent={accent}
		              style:--radius={radiusValue}
		              style:--scale={scaleValue}
		            Every CSS var() below in the <style> block reads these.
	-->
	<section class={{ preview: true, dark: mode === 'dark' }}>
		...
		<div class="preview-card">
			...
			<div class="row">
				<!--
					Lesson 08 example: class composition with the array form.
					Falsy entries (false, undefined, '') drop out; strings are joined.
					This is the shape you'll use when a parent component forwards
					a `class` prop of its own (Lesson 11's territory).
				-->
				<button
					type="button"
		...
		<!-- Lesson 08 build: wrap this toast in {#if showToast}…{/if}. -->
