<script lang="ts">
	...
	function handleWindowKeydown(event: KeyboardEvent): void {
		// Lesson 09 build: if the key is "d" or "D" (and the event target isn't
		// an input/textarea), preventDefault() and toggleMode().
	}
</script>
...
<!--
	Lesson 09 build:
	  - class: use the object form to apply `.dark` conditionally on mode==='dark'.
	    e.g. class={{ gallery: true, dark: mode === 'dark' }}
	  - style: inject the density token as a CSS custom property.
	    e.g. style:--density={densityValue}
-->
<div class="gallery">
	<header class="top">
		<div>
	...
	<section class="section">
		...
		<!--
			Lesson 09 build: the `.prose` wrapper is scoped to this file, but
			the elements inside come from {@html} — they have no hash class.
			The `.prose :global(...)` rules below style them. Leave the rules
			as-is; just verify the content renders with the right styles.
		-->
		<div class="prose">
			{@html html}
		</div>
...
<style>
	...
	/*
		Dark-mode token overrides. These apply when the `.gallery` root
		gets the `.dark` class (see the class stub above). Descendants inherit
		through var() — sub-components don't need their own dark rules.
	*/
	.gallery.dark {
